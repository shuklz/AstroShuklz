...
def _generate_hindi_pdf(chart, today, strength_data=None):
    ...
    # ── Hindi Forecast Reading Pages (full personalised readings) ──
    for period_key in ["week", "month", "year"]:
        hi_reading_data = generate_personalized_reading(bd, period=period_key, lang="hi")
        html_parts.append('<div class="page-break"></div>')
        html_parts.append(f'<h1 style="text-align:center; color:#8B0000;">{hi_reading_data["title"]}</h1>')
        html_parts.append('<div class="brand">by AstroShuklz</div>')
        for section in hi_reading_data['sections']:
            if section['heading']:
                html_parts.append(f'<h3>{section["heading"]}</h3>')
            html_parts.append(f'<div class="reading">{section["content"]}</div>')
        html_parts.append(
            '<div style="text-align:center; margin-top:15px; font-style:italic; '
            'color:#777; font-size:8.5pt;">'
            'यह पठन PDF निर्माण के समय की ग्रह स्थिति दर्शाता है। '
            'नवीनतम वर्तमान पठन के लिए <b>astroshuklz.com</b> पर जाएँ या '
            '<b>astroshuklz@shuklz.com</b> पर ईमेल करें।</div>')
...
def generate_pdf_to_buffer(chart, svg_content=None):
    ...
    # ── Forecast Reading Pages (full personalised readings) ─────
    import re as _re

    def _html_to_rl(html_content):
        """Convert HTML content from generate_personalized_reading to ReportLab-safe markup."""
        text = html_content
        # Convert <div> and <p> to line breaks
        text = _re.sub(r'</?div[^>]*>', '<br/>', text)
        ...
        # Strip class attributes and style attributes from remaining tags
        text = _re.sub(r'\s+class="[^"]*"', '', text)
        text = _re.sub(r'\s+style="[^"]*"', '', text)
        # Normalise all <br> variants to <br/>
    ...
    forecast_title_style = ParagraphStyle('ForecastTitle', parent=styles['Normal'],
        fontName='Helvetica-Bold', fontSize=18, textColor=MAROON,
        alignment=TA_CENTER, spaceAfter=2*mm, spaceBefore=5*mm)
    forecast_subtitle_style = ParagraphStyle('ForecastSub', parent=styles['Normal'],
        fontName='Helvetica-Oblique', fontSize=10, textColor=colors.HexColor("#B8860B"),
    ...
    for period_key, period_label in [("week", "Weekly"), ("month", "Monthly"), ("year", "Yearly")]:
        ...
        story.append(Paragraph(
            ...
            "email <b>astroshuklz@shuklz.com</b>",
            forecast_stale_style))
